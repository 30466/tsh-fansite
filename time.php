...
function beijingTimeZone(): DateTimeZone {
    static $zone = null;
    if ($zone === null) $zone = new DateTimeZone(BEIJING_TIME_ZONE);
    return $zone;
}
...
function beijingDateTimeFromUnixMs($value): ?DateTimeImmutable {
    ...
    try {
        return (new DateTimeImmutable('@' . $seconds))->setTimezone(beijingTimeZone());
...
function archiveDateFromUnixMs($value): ?string {
    $date = beijingDateTimeFromUnixMs($value);
    if ($date === null) return null;
    if ((int) $date->format('G') < ARCHIVE_DAY_START_HOUR) $date = $date->modify('-1 day');
    return $date->format('Y-m-d');
}
