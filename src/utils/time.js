...
export function parseBeijingDateTime(value) {
  ...
  const validDate = probe.getUTCFullYear() === parts.year &&
    probe.getUTCMonth() + 1 === parts.month && probe.getUTCDate() === parts.day
  const validTime = parts.hour >= 0 && parts.hour <= 23 &&
    parts.minute >= 0 && parts.minute <= 59 && parts.second >= 0 && parts.second <= 59
...
export function formatBeijingDateTime(value, { seconds = true } = {}) {
  const parts = getBeijingParts(value)
  if (!parts) return ''
  return `${formatCivilDate(parts)} ${formatBeijingTime(value, { seconds })}`
}

export function getArchiveDate(value) {
  const parts = getBeijingParts(value)
  if (!parts) return null
  return parts.hour < ARCHIVE_DAY_START_HOUR
