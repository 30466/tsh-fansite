...
const songRows = data => {
  if (!Array.isArray(data)) throw new Error('songs.json 顶层必须是数组')
  return data
}
const videoRows = data => {
  if (!data || !Array.isArray(data.clips) || data.totalClips !== data.clips.length) throw new Error('videoclips.json 结构或总数无效')
  return data.clips
}
const sources = [
