...
const UP_LIST = '/Users/cbj/Documents/code/bili-core/tsh-up-list.txt';
...
for (const upDir of upDirs) {
  ...
  for (const file of files) {
    ...
    for (const [bvid, video] of entries) {
      ...
      // 过滤：标题或简介任一包含关键词即可保留
      // 对 UP 主“谭思慧在此”不进行筛选，保留其全部视频
      if (upName !== '谭思慧在此' && !title.includes(FILTER_KEYWORD) && !description.includes(FILTER_KEYWORD)) continue;
